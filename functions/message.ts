...
export async function memberChannelMove (oldState: VoiceState, newState: VoiceState, dataFile: string) {
  try {
    if (!newState) return
    await wait(5000)
...
export async function startMessage (client: Client, guildId: string, dataFile: string) {
  ...
  let list = Object.keys(membersWaiting)
  ...
  for (const i in list) {
    ...
    const qMember = membersWaiting[list[i]].member
    if (!qMember.voice.channel) return
    if (!qMember.voice.channel.name.includes('┊')) return
    const qChannel = qMember.voice.channel
